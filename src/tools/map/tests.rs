...
async fn test_map_fragment_anchor() {
    ...
    let urls = map_page(&html.into(), "https://example.com/page").await;
    assert_eq!(urls.len(), 1);
...
async fn test_map_relative_and_absolute() {
    ...
    let urls = map_page(&html.into(), "https://example.com/base/").await;
    assert_eq!(urls.len(), 3);
...
async fn test_map_invalid_base() {
    ...
    let urls = map_page(&html.into(), "not-a-url").await;
    assert_eq!(urls.len(), 0);
}
...
async fn test_map_children() {
    ...
    let urls = map_children(&html.into(), "https://example.com").await;
    assert_eq!(urls.len(), 2);
